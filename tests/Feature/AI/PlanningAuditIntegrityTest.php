...
class PlanningAuditIntegrityTest extends PedagogyTestCase
{
    ...
    public function test_import_normal_satisface_guard_y_permita_decision_posterior_en_misma_fuente(): void
    {
    ...
    {
        foreach ([PromptCategory::Generation, PromptCategory::Audit] as $category) {
            $key = $category === PromptCategory::Generation ? 'planning.generation' : 'planning.audit';
            $template = PromptTemplate::factory()->create(['key' => $key, 'category' => $category->value]);
            if ($category === PromptCategory::Generation) {
                $attributes = [
