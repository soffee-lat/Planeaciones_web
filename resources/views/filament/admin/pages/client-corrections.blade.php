<x-filament-panels::page>
    @php
        ...
        $processing = $this->processingCorrections();
    ...
    <x-filament::section>
        ...
        <div class="grid gap-4 md:grid-cols-4">
            ...
            <div class="rounded-xl border border-primary-200 bg-primary-50 p-4 dark:border-primary-800 dark:bg-primary-950/30">
                <div class="text-xs font-semibold uppercase tracking-wide text-primary-700 dark:text-primary-300">En corrección</div>
                <div class="mt-1 text-3xl font-semibold">{{ $processing->count() }}</div>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">Revisiones aceptadas que ya están dentro del pipeline.</p>
            </div>
    ...
    <x-filament::section>
        ...
        <x-slot name="description">
            Una revisión aceptada no desaparece: permanece aquí hasta que la corrección se aplique y pase por reauditoría.
        </x-slot>

        @if ($processing->isEmpty())
            <p class="text-sm text-gray-500">No hay revisiones de clientes en procesamiento.</p>
        @else
            <div class="space-y-5">
                @foreach ($processing as $correction)
                    <div class="rounded-xl border-2 border-primary-200 bg-white p-5 shadow-sm dark:border-primary-800 dark:bg-gray-900">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span class="text-lg font-semibold">Revisión #{{ $correction->id }}</span>
                                    <span class="rounded-full bg-primary-100 px-2.5 py-1 text-xs font-semibold text-primary-800 dark:bg-primary-900 dark:text-primary-200">
                                        {{ $this->statusLabel($correction->status) }}
                                    </span>
                                </div>
                                ...
                                <div class="mt-3 grid gap-x-8 gap-y-1 text-sm text-gray-600 md:grid-cols-2 dark:text-gray-300">
                                    <div><span class="font-medium">Cliente:</span> {{ $correction->requester?->name ?? $correction->request?->owner?->name ?? '—' }}</div>
                                    <div><span class="font-medium">Planeación:</span> #{{ $correction->request_id }} · {{ $correction->request?->project ?? '—' }}</div>
                                    <div><span class="font-medium">Motivo:</span> {{ $this->reasonLabel($correction) }}</div>
                                    <div><span class="font-medium">Versión origen:</span> #{{ $correction->source_version_id }}</div>
                                </div>
                                ...
                                <div class="mt-4 rounded-lg border border-primary-100 bg-primary-50 p-4 dark:border-primary-900 dark:bg-primary-950/30">
                                    <div class="text-xs font-semibold uppercase tracking-wide text-primary-700 dark:text-primary-300">Solicitud original del cliente</div>
                                    <p class="mt-1 whitespace-pre-wrap text-sm text-gray-800 dark:text-gray-100">{{ $correction->description }}</p>
                                </div>
                            </div>

                            <div class="shrink-0">
                                <x-filament::button tag="a" color="primary" href="{{ $this->aiOperationsUrl() }}">
                                    Continuar en Operación IA
                                </x-filament::button>
                                <p class="mt-2 max-w-52 text-xs text-gray-500">
                                    Operación IA mostrará este mismo comentario y limitará la corrección a las secciones autorizadas.
                                </p>
                            </div>
                        </div>
                    </div>
