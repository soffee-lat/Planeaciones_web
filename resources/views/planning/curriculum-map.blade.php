...
@php
    ...
    $editUrl = \App\Filament\App\Resources\PlanningRequests\PlanningRequestResource::getUrl('edit', ['record' => $request]);
@endphp
...
<main class="wrap">
    <div class="progress" aria-label="Progreso de la planeación">
        <div class="step done"><span class="num">✓</span><div><strong>Datos</strong><br><span>Grupo, fechas, tema y formato</span></div></div>
        ...
        <aside class="aside">
            <section class="section">
                ...
                <form method="POST" action="{{ route('planning.curriculum-map.confirm', $request) }}">
                    @csrf
                    <button class="btn primary confirm" type="submit" @disabled($pending_count > 0 || count($selected['pdas']) === 0)>
                ...
                <a class="advanced-link" href="{{ $editUrl }}?paso=2">Editar detalles avanzados antes de confirmar</a>
            </section>
        </aside>
    </div>
</main>
