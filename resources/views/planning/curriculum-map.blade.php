...
<main class="wrap">
    ...
    <div class="grid">
        ...
        <aside class="aside">
            ...
            <section class="section">
                <h3>Cobertura del horario</h3>
                <div class="muted">Cada campo oficial no flexible que aparece en tu horario necesita al menos un contenido curricular y un PDA seleccionados.</div>
                ...
                @if(($schedule_field_coverage['missing'] ?? []) !== [])
                    <div class="pending-warning">
                        Antes de confirmar agrega contenido y PDA de los campos marcados como faltantes. Si no, la IA no podría respetar al mismo tiempo tu horario y el currículo.
                    </div>
                @endif
            </section>
            @endif
            ...
            <section class="section">
                ...
                <form method="POST" action="{{ route('planning.curriculum-map.confirm', $request) }}">@csrf
                    <button class="btn primary confirm" type="submit" @disabled(($schedule_field_coverage['missing'] ?? []) !== [])>Confirmar estas conexiones</button>
                </form>
            </section>
        </aside>
    </div>
