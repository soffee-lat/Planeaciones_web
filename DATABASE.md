...
### Materialización Fase 4C

- `documents`: una fila lógica por `planning_request` (`request_id UNIQUE`), owner congelado, título y `current_version_id` nullable. FK compuesta `(request_id,owner_id)` evita documentos ajenos; identidad y borrado están protegidos. `current_version_id` solo puede avanzar a una versión posterior del mismo documento.
- `document_versions`: `document_id`, `number`, `parent_version_id`, `input_revision`, `content JSONB` (`CanonicalPlanV1`), `content_hash`, `source_payload_hash`, `created_by`, `ai_execution_id`, `status`, `created_at`. UNIQUE(document_id,number) y UNIQUE(ai_execution_id). Toda fila es append-only. Parent y current version usan FK compuestas al mismo documento.
- `ai_executions.resulting_version_id` obtiene FK real a `document_versions`. Para generation/correction, `status=succeeded` exige versión resultante y `finished_at`; provider/model son ambos null o ambos reales, y `actual_cost` exige moneda ISO. Una ejecución succeeded no puede reescribir su resultado ni tiempos de cierre. Si provider/model o actual_cost/currency quedaron null al importar, pueden completarse una sola vez cuando llegue la conciliación real; desde que existen, esos metadatos históricos quedan inmutables.
...
- `planning_audit_result_integrity_*` exige desde `AUDITORIA_IA` outbox audit sobre la `DocumentVersion` actual; si la ejecución está `waiting_manual`/`succeeded` debe existir paquete privado. Para cualquier estado posterior a `AUDITORIA_IA` exige una ejecución audit `succeeded` con reporte sobre la versión/hash vigente.

### Materialización Fase 4E

- `approvals`: `request_id`, `version_id`, `kind`, `ai_execution_id`, `review_id`, `actor_id`, `approved_at`. UNIQUE(version_id,kind), append-only. `kind=ai` referencia auditoría `succeeded/passed=true`; desde 5B `kind=human` referencia una `reviews.id` aprobada sobre la misma `DocumentVersion`, con actor=revisor y checklist obligatorio completo.
- `CorrectionResultV1` se versiona en `resources/schemas/ai/correction_result_v1.schema.json`. Solo contiene `source_version_id` y reemplazos de raíces explícitamente mutables. `source`, `context` y `curricular_alignment` no forman parte del schema de patch; `planning` restringe cambios a `title`/`project_name`. Después del schema, el servidor vuelve a validar draft/canonical y el scope derivado del AuditResult.
- `ai_executions` stage `correction` usa manifest v1 con versión/hash fuente, audit execution/hash exactos, `section_keys`, `correction_round` y correlation_id. CHECK PostgreSQL valida su forma. `outbox_events` incorpora `planning.correction.requested`; el paquete usa `ai_manual_packages` y almacenamiento privado existente.
...
- reviewer_work_items: request_id, review_id UNIQUE, assignment_id UNIQUE, reviewer_id, cycle, rate_minor (por unidad), quantity, total_minor, currency, status (pending/approved/paid), approved_at, paid_at, settlement_id nullable; UNIQUE(request_id,cycle). Un trabajo pagable por ciclo aprobado, no por cada vuelta interna.
- reviewer_settlements: reviewer_id, currency, status (draft/approved/paid), reference UNIQUE nullable, approved_by/approved_at, paid_by/paid_at. Total deriva de trabajos; un settlement abierto por revisor+moneda e idempotencia al marcar pagado. Sin nómina ni transferencia automática.
