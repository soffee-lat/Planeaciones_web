...
- `approvals`: `request_id`, `version_id`, `kind`, `ai_execution_id`, `review_id`, `actor_id`, `approved_at`. UNIQUE(version_id,kind), append-only. En 4E solo `kind=ai` es operacional y debe referenciar una auditoría `succeeded/passed=true` sobre esa misma versión; `kind=human` queda bloqueado en PostgreSQL hasta que Fase 5 provea `reviews`/checklist verificables.
